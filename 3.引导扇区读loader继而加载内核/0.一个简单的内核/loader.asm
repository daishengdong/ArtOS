...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
    ...
    mov di, OffsetOfKernelFile              ; es:di -> OffsetOfKernelFile
    cld
    mov dx, 10h                             ; 根目录区中一个条目 32 字节
                                            ; 一个扇区共 16(10h) 个条目
LABEL_SEARCH_FOR_KERNELBIN:
    cmp dx, 0                               ; 循环次数控制
    jz LABEL_GOTO_NEXT_SECTOR_IN_ROOT_DIR   ; 如果已经读完了一个 Sector
    dec dx                                  ; 就跳到下一个 Sector
    mov cx, 11                              ; 文件名 8 字节, 扩展名 3 字节
LABEL_CMP_FILENAME:
    cmp cx, 0
    ...
    lodsb                                   ; ds:si -> al
    cmp al, byte [es:di]
    jz LABEL_GO_ON                          ; 如果比较了 11 个字符都相等, 表示找到
...
bOdd                db  0                   ; 奇数还是偶数
dwKernelSize        dd  0                   ; KERLEN.BIN 文件大小

; 字符串
KernelFileName      db  "KERNEL  BIN", 0    ; KERNEL.BIN 之文件名
; 为简化代码, 下面每个字符串的长度均为 MessageLength
MessageLength       equ 9
LoadMessage         db  "loading  "         ; 9 字节, 不够用空格补齐, 序号 0
Message1            db  "Ready.   "         ; 9 字节, 不够用空格补齐, 序号 1
Message2            db  "NO KERNEL"         ; 9 字节, 不够用空格补齐, 序号 2


;----------------------------------------------------------------------------
; 实模式下显示字符串的函数
; 保护模式下显示字符串由函数 DispStr 完成
...
LABEL_PM_START:
	...
	; KERNEL 的位置实际上是很灵活的，可以通过同时改变 LOAD.INC 中的
...
.1:                             ;
    ...
    jne .2                      ;   {
    mov eax, [dwBaseAddrLow]    ; 因为是 x86 平台, 所以只用到低 32 位
    add eax, [dwLengthLow]      ;
...
.1:
    stosd                       ; es:edi <- eax
                                ; 每个页目录表现 4 个字节, 所以用 stosd 双字长, 每次 edi 自动加 4
...
; 遍历每一个 Program Header, 根据 Program Header 中的信息来确定把什么放进内存
; 放到什么位置, 以及放多少
InitKernel:
    xor esi, esi
...
.Begin:
    mov eax, [esi + 0]
    cmp eax, 0                                  ; PT_NULL
    jz .NoAction                                ; pPHdr->p_type == 0 -> 该段不 load
    ; memcpy((void*)(pPHdr->p_vaddr),
    ;       uchCode + pPHdr->p_offset,
    ;       pPHdr->p_filesz)
    push dword [esi + 010h]
    mov eax, [esi + 04h]                        ; 该段在文件中的偏移
