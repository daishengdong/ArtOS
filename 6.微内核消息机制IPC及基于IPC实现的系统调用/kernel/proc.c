...
// ----------------------------------------------------------------------------
// 每个进程都有自己的 LDT, 位于进程表的中间
// 这个函数根据 LDT 中描述符的索引来求得描述符所指向的段的基地址
/**
...
PRIVATE int msg_receive(struct proc *current, int src, MESSAGE *m) {
    struct proc *p_who_wanna_recv = current;    /**
                                                 ...
                                                 * wired, but it makes me think clearly, so I keep
                                                 * it.
                                                 */
